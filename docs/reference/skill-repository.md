...
- 이 저장소는 `justified-change`라는 독립 스킬 패키지다.
- 기본 README 언어는 한국어이고, 영어 문서는 `README.en.md`에 둔다.
- 설치 가능한 스킬은 `skills/justified-change/` 아래에 둔다.
- `SKILL.md`는 영문 base skill이며 frontmatter metadata를 가진다.
- `SKILL.ko.md`는 한국어 페어이며 frontmatter를 넣지 않는다.
- `skills/justified-change/agents/openai.yaml`은 OpenAI-facing interface metadata를 제공한다.
- 프로젝트 라이선스는 MIT다.
...
- 스킬 본문은 독립적으로 유지한다. sibling skill은 README에서 소개할 수 있지만 `SKILL.md`에서 호출하거나 의존하지 않는다.
